...
#TODO:Fix
        ...
        """ Feedback function """
        mean_s = ca.MX.sym('mean', Ny) # mean state
        v_s = ca.MX.sym('v', Nu) # raw inputs
        if feedback:
            u_func = ca.Function('u', [mean_s, mean_ref_s, v_s, K_s],
                                 [v_s + ca.mtimes(K_s.reshape((Nu, Ny)),
                                 mean_s-mean_ref_s)]) # feedback ie. u = K(error: state-ref) + v
        else:
        ...
        # This allows you to create a structured representation of variables in CasADi, which can be more convenient.
        # Theses are all for the optimization problem specifically as opposed to appropriate functions as before?
        ...
        num_state_slack = Ny
        self.__var = var # stores the structured representation of the decision variables
        ...
        # adjust boundaries for decision variables in the optimisation problem. 
        for t in range(Nt + 1):
            j = Ny
            k = 0
            for i in range(Ny):
        ...
        if discrete_method is 'hybrid':
            N_gp, Ny_gp, Nu_gp = self.__gp.get_size() # number of training data, number of outputs, number of control inputs
            Nz_gp = Ny_gp + Nu_gp
            covar_d_sx = ca.SX.sym('cov_d', Ny_gp, Ny_gp) # symbolic matrix representing covariance of dynamic states
            K_sx = ca.SX.sym('K', Nu, Ny) # symbolic matrix representing the feedback gain
            covar_u_func = ca.Function('cov_u', [covar_d_sx, K_sx],
#                                       [K_sx @ covar_d_sx @ K_sx.T])
                                        [ca.SX(Nu, Nu)]) # function to calculate the covariance of the control inputs
            covar_s = ca.SX(Nz_gp, Nz_gp)
            covar_s[:Ny_gp, :Ny_gp] = covar_d_sx
#            covar_s = ca.blockcat(covar_x_s, cov_xu, cov_xu.T, cov_u)
            covar_func = ca.Function('covar', [covar_d_sx], [covar_s], {'allow_free': True})
    
    #   This has been commented out as I dont think i am going to use it. 
    ...
    #             covar_func = ca.Function('covar', [covar_d_sx], [covar_s], {'allow_free': True})
            ...
            cov_u = covar_u_func(covar_x_s, K_s.reshape((Nu, Ny)))
            covar_s = ca.blockcat(covar_x_s, cov_xu, cov_xu.T, cov_u)
            # Overall aim, take dynamic state, combines with control input, and then combines with the covariance of the control input
            # returns combined covariance matrix
            # Ask Dario
            covar_func = ca.Function('covar', [covar_x_s], [covar_s], {'allow_free': True})
    ...
    # This has been commented out as I don't think I am going to use it. 
...
#                B = self.__Bf @ B_f + self.__Bd @ B_gp
        ...
        if costFunc is 'quad':
            self.__l_func = ca.Function('l', [mean_s, covar_x_s, u_s,
                               ...
                               [self.__cost_l(mean_s, mean_ref_s, covar_x_s, u_s,
                                covar_u_s, delta_u_s, Q, R, S)], {'allow_free': True})
            self.__lf_func = ca.Function('lf', [mean_s, covar_x_s, P_s],
                                   [self.__cost_lf(mean_s, mean_ref_s, covar_x_s, P_s)], {'allow_free': True})
    ...
    def plot(self, title=None,
        ...
        if title is not None:
            fig_x.suptitle(title)  # Use suptitle instead of set_window_title
        else:
            fig_x.suptitle(('MPC Horizon: {x}, Feedback: {y}, '
                                           'Discretization: {z}'
                                           ).format( x=self.__Nt,
                                                     y=self.__feedback,
...
def plot_eig(A, discrete=True):
    ...
    fig.suptitle('Eigenvalues of linearized system')
    plt.show()
    return eigenvalues
