...
class RLExample:
    ...
    def train_with_eval(self, load_checkpoint=False, ngames=2000, max_steps_per_episode=300, sigma_noise=1.5, gamma=0.99, theta_noise=0.15):
        ddpg_controller = DDPGTrainer(self.env, model_name=self.model_name, 
                                    load_checkpoint=load_checkpoint, ngames=ngames, max_steps_per_episode=max_steps_per_episode, sigma_noise=sigma_noise,
                                    sigmoid=self.sigmoid, action_bound=self.action_bound, input_dims=self.input_dims, n_actions=self.n_actions, problem=self.problem, dt=self.dt, gamma=gamma, theta_noise=theta_noise)
        ddpg_controller.run(eval=True)


    def evaluate(self, total_simulation_time=20, state_labels=None, control_labels=None):
    ...
    def plot_average_stats(self, state_labels=None, control_labels=None):
        ...
        if os.path.exists(evaluation_stats_file_path):
            ...
            plot_average_errors_by_state(average_errors_by_state, std_errors_by_state, episode_number)
            plot_average_controls_by_input(average_controls_by_input, std_controls_by_input, episode_number)

            plot_total_average_errors(total_average_errors, total_std_errors, episode_number)
            plot_total_average_control(total_average_controls, total_std_controls, episode_number)
