...
def train_four_tank():
    ...
    ddpg_controller = DDPGTrainer(env, model_name="four_tank", load_checkpoint=False, input_dims=[4], n_actions=2, ngames=2000, max_steps_per_episode=300, sigmoid=True, action_bound=100)
    ddpg_controller.run()


def evaluate_four_tank():
    env = CustomFourTankEnv()
    ddpg_evaluator = DDPGEvaluator(env, model_name="four_tank", input_dims=[4], n_actions=2, dt=3, total_simulation_time=240, sigmoid=True, action_bound=100)
...
def plot_rewards_four_tank():
    rewards_file_path = os.path.join(os.path.dirname(__file__), 'episode_rewards/four_tank_e_r.npy')
