...
class MPCController:
    ...
    def output(self, return_x_seq=False, return_u_seq=False, return_eps_seq=False, return_status=False,
        ...
        """ Return the MPC controller output uMPC, i.e., the first element of the optimal input sequence and
            assign is to self.uminus1_rh.
    ...
    def _update_QP_matrices_(self):
        ...
        (Np + 1) * nx + (Np + 1) * nx + (Nc) * nu:(Np + 1) * nx + (Np + 1) * nx + (Nc) * nu + nu] = Dumin + uminus1_rh[
                                                                                                            0:nu]
        # update constraint on \Delta u0: Dumin <= u0 - u_{-1}
        self.u[
        (Np + 1) * nx + (Np + 1) * nx + (Nc) * nu:(Np + 1) * nx + (Np + 1) * nx + (Nc) * nu + nu] = Dumax + uminus1_rh[
                                                                                                            0:nu]
        # update constraint on \Delta u0: u0 - u_{-1} <= Dumax
        ...
        if self.JU_ON:
            ...
            else:  # Nc < Np. This formula is more general and could handle the case Nc = Np either.
                iU = np.ones(Nc)
                iU[Nc - 1] = (Np - Nc + 1)
    ...
    def _compute_QP_matrices_(self):
        ...
        iBu = sparse.vstack([sparse.csc_matrix((1, Nc)),
                             sparse.eye(Nc)])
        if self.Nc < self.Np:  # expand A matrix if Nc < Np (see notes)
        ...
        uineq_du = np.kron(np.ones(Nc + 1), Dumax)
        uineq_du[0:nu] += self.uminus1[0:nu]

        lineq_du = np.kron(np.ones(Nc + 1), Dumin)
...
if __name__ == '__main__':
    ...
    uminus1 = np.array([0.0])  # input at time step negative one -
    # used to penalize the first delta u at time instant 0. Could be the same as uref.
