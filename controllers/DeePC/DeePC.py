...
class npDeePC:
    ...
    def __init__(self, ud: np.ndarray, yd: np.ndarray, y_constraints: Tuple[np.ndarray],
        ...
        U = block_hankel(w=ud.reshape((m * self.T,)), L=Tini + N, d=m)  # Input hankel matrix w input as column vector
        Y = block_hankel(w=yd.reshape((p * self.T,)), L=Tini + N, d=p)  # Output hankel matrix w input as column vector

        self.Up = U[0:m * Tini, :]  # Past input trajectory
        self.Yp = Y[0:p * Tini, :]  # Past output trajectory
        self.Uf = U[Tini * m:, :]  # Future input trajectory
        self.Yf = Y[Tini * p:, :]  # Future output trajectory
