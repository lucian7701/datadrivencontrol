...
class Model:
    def __init__(self, Nx, Nu, ode, dt, R=None,
        ...
        dae = {'x': x, 'ode': ode(x,u,z,p), 'p':par} # setting up a Differential-Algebraic Equation (DAE)
        ...
        if alg is not None:
            ...
            # integrates based on integrator options
...
#TODO: Fix this or remove
    ...
    def generate_training_data(self, N, uub, ulb, xub, xlb,
        ...
        # Generate a set of control inputs within the specified bounds. 
        if self.__Nu > 0:
            # maximin spreads out points as evenly as possible
            U = pyDOE.lhs(self.__Nu, samples=N, criterion='maximin')
             # Scale control inputs to correct range, originally these inputs are between 0 and 1. 
            for k in range(N):
                U[k, :] = U[k, :] * (uub - ulb) + ulb
        else:
            U = []
        ...
        # Simulate the system, get the output for given state and control inputs
        for i in range(N):
            if self.__Nu > 0:
